...
int main() {
  ...
  const char gbkData[] = "\xb2\xe2\xca\xd4\xd5\xe2\xca\xc7\xd2\xbb\xb6\xceGBK"
                         "\xb1\xe0\xc2\xeb\xb5\xc4\xce\xc4\xb1\xbe";
  uchardet_handle_data(ud, gbkData, sizeof(gbkData) - 1);
  uchardet_data_end(ud);
  const char *charset = uchardet_get_charset(ud);
  std::cout << "uchardet: " << (charset ? charset : "unknown") << "\n";
  uchardet_delete(ud);

  // 4. ICU
  UVersionInfo icuVersion;
  u_getVersion(icuVersion);
  char icuStr[U_MAX_VERSION_STRING_LENGTH];
  u_versionToString(icuVersion, icuStr);
  std::cout << "ICU: " << icuStr << "\n";
  ...
  std::cout << "FFmpeg avformat: " << AV_VERSION_MAJOR(avformatVer) << "."
            << AV_VERSION_MINOR(avformatVer) << "."
            << AV_VERSION_MICRO(avformatVer)
            << ", avcodec: " << AV_VERSION_MAJOR(avcodecVer) << "."
            << AV_VERSION_MINOR(avcodecVer) << "."
            << AV_VERSION_MICRO(avcodecVer) << "\n";
  ...
  // 8. QtKeychain
  QKeychain::ReadPasswordJob job(QStringLiteral("AiMusic"));
